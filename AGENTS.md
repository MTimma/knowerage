...
1. **Create analysis** — `knowerage_create_or_update_doc` with YAML frontmatter (`source_file`, `covered_lines`, `analysis_date`).
2. **Reconcile** — `knowerage_reconcile_record` after every create or update to that analysis file (required).
3. **Coverage** — Reconcile updates `knowerage/registry.json`; no manual registry edits.

For a full registry snapshot (same shape as `registry.json`), use `knowerage_list_registry` instead of opening the file by hand. **“Analyze X”** means the full workflow above; the user does not need to say “record coverage” or “use the MCP.”
